...
INNER JOIN
    ...
    ON
        b.memid = m.memid
WHERE 
    SUBSTRING(b.starttime, 1, 10) = '2012-09-14'
ORDER BY 
    price DESC
...
(
    SELECT 
    	b.facid AS facid1,
        ((COUNT(b.facid) * f1.guestcost) + (COUNT(b.facid) * f1.membercost)) AS total_rev
    FROM
        Bookings AS b
    INNER JOIN
        Facilities AS f1
...
SELECT
    f.name,
    EXTRACT(MONTH FROM SUBSTRING(b.starttime, 1, 10)) AS mon,
    COUNT(b.memid)
FROM
    Facilities AS f
INNER JOIN
